...
summary: '"如何在 Next.js 项目中使用 Supabase"是一篇关于构建现代 Web 应用的教程，其中详细介绍了如何在你的 Next.js 应用程序中集成Supabase，这是一个开源后端即服务平台，可以提供认证、实时数据库和存储等功能。通过本文，你将学会如何创建 Supabase 帐户、初始化应用程序、进行身份验证并与其 API 进行交互。此外，你还将了解到如何使用 Supabase 实现实时数据更新以及如何处理表格的 CRUD 操作。如果你正在寻找一种简单而可靠的方式来管理你的应用程序后端，那么这篇文章是值得一读的。'
tag: 'Web 开发'
category: 'blog'
---
...
新建使用 insert，更新使用 update。这样就可以讲阅读量存储到 supabase 中
